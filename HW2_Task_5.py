...
moi_spisok = [7, 5, 3, 3, 2]
request = int(input('Введите целое число: '))
for index, number in enumerate(moi_spisok):
    if request < int(number):
        continue
    moi_spisok.insert(index, request)
    break
else:
    moi_spisok.append(request)
print(moi_spisok)


# Способ 2й через методы  и функцию sorted()

moi_spisok = [7, 5, 3, 3, 2]
request = int(input('Введите целое число: '))
moi_spisok.append(request)
print(sorted(moi_spisok, reverse=True))
